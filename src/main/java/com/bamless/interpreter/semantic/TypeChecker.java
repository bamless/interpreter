...
public class TypeChecker implements GenericVisitor<Type, FuncDecl> {
	...
	public Type visit(ArrayDecl a, FuncDecl currentFunc) {
		...
		for(Expression e : a.getDimensions()) {
			...
			if(ind != Type.INT)
				typeError(e.getPosition(), "cannot convert from %s to int",
						ind.toString().toLowerCase());
		}
	...
	public Type visit(ReturnStatement r, FuncDecl currentFunc) {
		Type exp = r.getExpression() == null ? Type.VOID
				: r.getExpression().accept(this, currentFunc);
		...
		if(currentFunc.getType().getWidenFactor() < exp.getWidenFactor()) {
			ErrUtils.warn(
					"Warning %s: implicit conversion from %s to %s, possible loss of precision",
	...
	public Type visit(ArithmeticBinExpression e, FuncDecl currentFunc) {
		...
		if(res == null) {
			undefOperatorError(e.getPosition(), e.getOperation().toString(),
	...
	public Type visit(LogicalExpression l, FuncDecl currentFunc) {
		...
		if(res == null) {
			undefOperatorError(l.getPosition(), l.getOperation().toString(),
					left.toString().toLowerCase(), right.toString().toLowerCase());
		}
	...
	public Type visit(RelationalExpression r, FuncDecl currentFunc) {
		...
		if(res == null) {
			undefOperatorError(r.getPosition(), r.getOperation().toString(),
					left.toString().toLowerCase(), right.toString().toLowerCase());
		}
	...
	public Type visit(PostIncrementOperation p, FuncDecl currentFunc) {
		...
		if(t != Type.INT && t != Type.FLOAT) {
			undefOperatorError(p.getPosition(), p.getOperator().toString(),
	...
	public Type visit(AssignExpression e, FuncDecl currentFunc) {
		...
		if(lval != expr) {
			e.setExpression(
					new CastExpression(lval, e.getExpression(), e.getExpression().getPosition()));
		}
	...
	public Type visit(FuncCallExpression f, FuncDecl currentFunc) {
		...
		if(callArgs.length != declArgsTypes.length) {
			ErrUtils.semanticError(f.getPosition(),
					"Function %s requires %d arguments, but instead %d supplied", f.getFuncName(),
					declArgsTypes.length, callArgs.length);
		}
		...
		for(int i = 0; i < callArgs.length; i++) {
			...
			if(!callType.isCompatible(declType)) {
				typeError(callArgs[i].getPosition(),
						...
						callType.toString().toLowerCase(), declType.toString().toLowerCase(),
						cardinal(i + 1), f.getFuncName());
			}
	...
	public Type visit(CastExpression c, FuncDecl arg) {
		...
		if(!c.getType().isCompatible(exprType)) {
			typeError(c.getPosition(), "Cannot cast from %s to %s",
					exprType.toString().toLowerCase(), c.getType().toString().toLowerCase());
		}
	...
	private void typeError(Position pos, String format, Object... currentFuncs) {
		throw new TypeException(
				String.format("Type error at " + pos + ": " + format, currentFuncs));
	}

	private void undefOperatorError(Position pos, String operator, String... types) {
		throw new TypeException(
				"Type error at " + pos + " : operator " + operator + " undefined for the type"
	...
	private void widen(BinaryExpression e) {
		...
		if(l == r || l.getWidenFactor() == Type.NULL_WIDENFACT
				|| r.getWidenFactor() == Type.NULL_WIDENFACT) {
			return;
		}
